...
"""Expert dataset loader for AMP (Adversarial Motion Priors).

Ported from the classic NVIDIA AMP implementation (``frog_rl.datasets.motion_loader``),
with the ``pybullet_utils.transformations`` / ``pose3d`` / ``motion_util`` dependencies
replaced by the self-contained :mod:`frog_rl.datasets.quaternions` module.
"""
...
from frog_rl.datasets.quaternions import quat_normalize, quat_slerp, standardize_quaternion


class AMPLoader:
    """Expert dataset providing AMP observations from a set of motion clips.

    Each motion file is a JSON dictionary with the following keys:

    - ``Frames``: list of frames, each a flat 61-vector of
      ``[root_pos(3), root_rot(4), joint_pos(12), foot_pos_local(12), lin_vel(3),
      ang_vel(3), joint_vel(12), foot_vel_local(12)]``.
    - ``MotionWeight``: sampling weight of the clip.
    ...
    def __init__(
        ...
        """Initialize the expert dataset from motion clips.

        Args:
            device: Device to place the tensors on.
            time_between_frames: Amount of time in seconds between transitions.
            data_dir: Directory containing the motion files. Defaults to "".
            preload_transitions: Whether to preload transitions. Defaults to False.
            num_preload_transitions: Number of transitions to preload. Defaults to 1000000.
            motion_files: List of motion file paths. Defaults to ``datasets/motion_files2/*``.
        """
        self.device = device
        ...
        for i, motion_file in enumerate(motion_files):
            ...
            with open(motion_file, "r") as f:
                ...
                for f_i in range(motion_data.shape[0]):
                    ...
                    root_rot = quat_normalize(torch.from_numpy(root_rot).to(self.device)).cpu().numpy()
                    root_rot = standardize_quaternion(torch.from_numpy(root_rot)).cpu().numpy()
                    motion_data[
                        f_i,
                        AMPLoader.POS_SIZE : (AMPLoader.POS_SIZE + AMPLoader.ROT_SIZE),
                    ] = root_rot

                # Remove the first 7 observation dimensions (root_pos and root_orn).
    ...
    def get_trajectory(self, traj_idx):
        """Return trajectory of AMP observations."""
        return self.trajectories_full[traj_idx]

    def get_frame_at_time(self, traj_idx, time):
        """Return frame for the given trajectory at the specified time."""
    ...
    def get_frame_at_time_batch(self, traj_idxs, times):
        """Return frames for the given trajectories at the specified times."""
    ...
    def get_full_frame_at_time(self, traj_idx, time):
        """Return full frame for the given trajectory at the specified time."""
        p = float(time) / self.trajectory_lens[traj_idx]
    ...
    def get_full_frame_at_time_batch(self, traj_idxs, times):
        """Return full frames for the given trajectories at the specified times."""
        p = times / self.trajectory_lens[traj_idxs]
        n = self.trajectory_num_frames[traj_idxs]
        idx_low, idx_high = np.floor(p * n).astype(np.int), np.ceil(p * n).astype(np.int)
    ...
    def get_full_frame(self):
        """Return random full frame."""
        traj_idx = self.weighted_traj_idx_sample()
        sampled_time = self.traj_time_sample(traj_idx)
        return self.get_full_frame_at_time(traj_idx, sampled_time)

    def get_full_frame_batch(self, num_frames):
        """Return random full frames (used for reference-state initialization)."""
        if self.preload_transitions:
    ...
    def blend_frame_pose(self, frame0, frame1, blend):
        ...
        blend_root_rot = standardize_quaternion(blend_root_rot)
    ...
    def feed_forward_generator(self, num_mini_batch, mini_batch_size):
        """Generate a batch of AMP transitions ``(s, s_next)``."""
